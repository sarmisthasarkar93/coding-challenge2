...
public class PokerSessionExceptionController extends RuntimeException{
    ...
    public final ResponseEntity<Object> handleSessionExpireException(SessionNotFoundException ex, WebRequest request) {
        List<String> details = new ArrayList<>();
        details.add(ex.getLocalizedMessage());
        ErrorResponse error = new ErrorResponse("Session has been expired", details);
        return new ResponseEntity(error, HttpStatus.NOT_FOUND);
    }

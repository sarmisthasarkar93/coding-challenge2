...
import com.planning.poker.exception.SessionExpireException;
...
import com.planning.poker.repository.VoteStatusRepository;
...
//		int count = 0;
		...
		memberUstory.setPokerSession(pokerSession);
	...
	@Override
	public int fetchVoteFinalResultForUserStory(String uStoryId,String session) {
		logger.info("fectVoteFinalResultForUserStory start");
		/*
		 * UserStory userStory = userStoryRepository.getById(uStoryId);
		 * 
		 * Set<Member> members = userStory.getMembers(); List<Member> memberList = new
		 * ArrayList<Member>(); memberList.addAll(members); Optional<Member> member =
		 * memberList.stream()
		 * .sorted(Comparator.comparingInt(Member::getVotePoint).reversed()).findFirst()
		 * ;
		 * 
		 * 
		 * int voteFinalResult = member.get().getVotePoint();
		 */
		PokerSession pokerSession = pokerSessionRepository.getById(session);
		
		if(pokerSession.getIsSessionAlive()) {
			UserStory userStory = userStoryRepository.getById(uStoryId);
			...
			userStory.setSizeOfTheStory(voteFinalResult);
			
			userStoryRepository.save(userStory);
			logger.info("fectVoteFinalResultForUserStory end",voteFinalResult);
			return voteFinalResult;
		}else {
			throw new SessionExpireException("Session has been expired!!!!");
		}
